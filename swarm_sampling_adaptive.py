...
def respawn(accumulator, old_solution, old_energy):
    ...
    if np.mean(recent_energy_vals) > 8 and rank !=0: # if samples are bad for past few iterations
        new_solution = np.random.randint(-100,100) # respawn at new initial point
        new_energy = energy(new_solution)
        respawned, old_solution, old_energy = metropolis_hastings(old_solution, new_solution, old_energy, new_energy, temp) # accept respawn if proposal is better

    return old_solution, old_energy, respawned


def swarm_sampling(energy, proposal, init_sol, epochs, temp, exchange_rate, respawn_rate):
    """
    ...
    for epoch in range(epochs):
        ...
        if total_exchange_attempts < len(exchange_rate) and epoch == exchange_rate[total_exchange_attempts] and epoch > 0: 
            comm.barrier()
...
if __name__ == '__main__':
    ...
    init_sol = np.random.randint(-100,100,size=num_chains)
    ...
    # temp_list = np.linspace(1,7.5,num_chains)
    # temp = temp_list[rank]

    temp_list = [1, 1.5, 3, 4.5, 6, 7.5]
    temp = temp_list[rank]

    exchange_interval = [np.round(50000 - 50000/x) for x in range(1,500)]
    exchange_interval = sorted(list(set(exchange_interval)))[1:-1]


    respawn_rate = 25

    accumulator, ratio_accept, ratio_exchange, ratio_respawn = swarm_sampling(energy, proposal, init_sol, epochs=num_epochs, temp=temp, exchange_rate=exchange_interval, respawn_rate=respawn_rate)
    print(f'Agent {rank} [T ={temp}] accepted: {ratio_accept*100:.2f}%, received: {ratio_exchange*100:.2f}%, respawned: {ratio_respawn*100:.2f}%')
    ...
    # writing files
    np.save(f'{data_folder}/process_{rank}.npy', accumulator)
